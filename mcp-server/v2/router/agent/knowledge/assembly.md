...
### `face`
`top/bottom/left/right/front/back` 表示零件可用的接觸面（通常是零件本體的外側面）。在 mate 時要決定：
- sourceFace（要被移動/旋轉的零件上，哪個面去對齊）
- targetFace（目標零件上，哪個面做為對齊面）
- 方向基準：優先使用 **target-part frame / capture frame**，不要把 world 軸當成固定語意；整體場景旋轉後 face 語意應保持一致。

### `method`
Anchor 解析方法（用來算接觸面的 anchor / normal）：
- `planar_cluster`：從幾何中找平面群，通常比 bbox 更能貼近「真正的接觸面」。
- ~~`extreme_vertices`~~：**暫時停用**（目前執行時會 fallback 到 `planar_cluster`，保留名稱只為舊資料相容）。
- `geometry_aabb`：幾何 bbox（可能會被凸出物干擾）。
...
- 方法選擇：若 bbox 因凸起造成貼合不到真正接觸面，優先嘗試 `planar_cluster` 或 `obb_pca`
...
- 方法：插槽/凹槽場景避免只用 `object_aabb`，可優先 `planar_cluster/obb_pca`
...
- 方法：避免只用 `object_aabb`（凸出物會讓 bbox 過大、導致「插不進去」），優先 `planar_cluster/obb_pca`
