...
export const MCPToolSchemas = {
  ...
  'mate.record_demonstration': {
    args: z.object({
      ...
      /** Serialized feature pairs from the chosen candidate */
      chosenFeaturePairs: z.array(z.object({
        sourceFeatureId: z.string(),
        sourceFeatureType: z.string(),
        targetFeatureId: z.string(),
        targetFeatureType: z.string(),
        compatibilityScore: z.number(),
        dimensionFitScore: z.number(),
        axisAlignmentScore: z.number(),
        notes: z.array(z.string()),
      })).optional(),
      /** Final transform applied to source part */
      finalTransform: z.object({
        translation: Vec3Schema,
        rotation: QuaternionSchema,
        approachDirection: Vec3Schema,
        method: z.string(),
  ...
  /**
   * query.candidate_detail — Retrieve full detail of a specific mating candidate.
   * The candidate must have been previously generated by query.generate_candidates
   * (stored in the session registry).
   */
  ...
  /**
   * action.solve_candidate — Compute the alignment solution for a specific candidate
   * without applying it to the scene.
   */
  ...
  /**
   * action.apply_candidate — Solve and optionally apply a mating candidate to the scene.
   */
  'action.apply_candidate': {
    args: z.object({
      ...
      targetPart: PartRefSchema,
      candidateId: z.string(),
      /** Apply the transform to the scene (default: true) */
      commit: z.boolean().optional().default(true),
      /** Push to undo history (default: true) */
      pushHistory: z.boolean().optional().default(true),
      /** Optional step label to record in assembly steps */
      stepLabel: z.string().optional(),
    }),
    result: makeToolResultSchema(z.object({
      applied: z.boolean(),
...
const typedRequestSchemas = [
  ...
  z.object({ tool: z.literal('query.candidate_detail'), args: z.object({ sourcePart: PartRefSchema, targetPart: PartRefSchema, candidateId: z.string() }), meta: ToolMetaSchema.optional() }),
