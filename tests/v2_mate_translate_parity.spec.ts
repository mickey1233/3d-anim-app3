...
test.describe('v2 mate translate parity', () => {
  test('chat mate translate matches Mate panel solver', async ({ page }) => {
    ...
    await page.getByTestId('chat-input').fill('mate part1 bottom to part2 top');
    ...
    const qDiff =
      Math.abs(chatTransform.quaternion[0] - manualTransform.quaternion[0]) +
      Math.abs(chatTransform.quaternion[1] - manualTransform.quaternion[1]) +
      Math.abs(chatTransform.quaternion[2] - manualTransform.quaternion[2]) +
      Math.abs(chatTransform.quaternion[3] - manualTransform.quaternion[3]);

    expect(posDiff).toBeLessThan(1e-3);
    expect(qDiff).toBeLessThan(1e-3);
  });
});
